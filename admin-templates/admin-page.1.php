 ...
 * @since    1.0.0
 */
if ( file_exists( CESQT_PLUGIN_PATH . 'util/util.php' ) ) {
	require_once( CESQT_PLUGIN_PATH . 'util/util.php' );
}
// if(!class_exists('Resultados_CESQT_Table')){
//     require_once( CESQT_PLUGIN_PATH . 'util/Resultados_CESQT_Table.php' );
// }
// if(!class_exists('Orgs_Resiliencia_Table')){
//     require_once( CESQT_PLUGIN_PATH . 'util/Orgs_Resiliencia_Table.php' );
// }
...
function render_cesqt_qi_admin() {
    ...
    render_cesqt_admin_org(get_user_hash());
...
function render_cesqt_admin_org($org_id=NULL) {
    ...
    if($org_id != NULL) {
        ...
        // print do_shortcode('[resultados-cuestionario-cesqt org_id="' . $org_id . '"]');
        print '</div>';
...
function cesqt_admin_tabs( $current = 'INFORMACION' ) {
    ...
    foreach( $grupos as $index => $row ){
        ...
        echo '<a class="nav-tab$class" href="?page=theme-settings&tab=$tab">' . $row['nombrelimpio'] . '</a>';
